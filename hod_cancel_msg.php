...
$row=mysqli_fetch_array($result);
	$status= $row[11];
        ...
        $tol= $row[6];
       $res=mysqli_query($con,"select * from leave_history where leave_history.staff_id='$hodid';");
...
if($todate<$today){
    echo'<script> alert("Leave already taken")</script>';
        header("refresh:0;url=hod_home.php");
}
else if($frmdate<=$today&&$todate>$today){
 
mysqli_query($con," UPDATE leave_requests SET status = 'hodcancelled',from_date = '$tomorrow',no_of_days = '$ddiff' WHERE app_no = '$appno';");   
...
<h5 style="text-align: left;color:gray">You will Be redirected to Home page in 5sec.....</h5>';
 if($status=="hod approved"){
    ...
    if($tol=="casual"){
        $ctak=$ctak-$nod;
        $crem=$crem+$nod;
        mysqli_query($con,"UPDATE `leave_history` SET `cas_taken` = '$ctak', `cas_remaining` = '$crem' WHERE `leave_history`.`staff_id` = '$hodid';");
    }
    else if($tol=="Half Pay"){
        $htak=$htak-$nod;
        $hrem=$hrem+$nod;
        mysqli_query($con,"UPDATE `leave_history` SET `half_taken` = '$htak', `half_remaining` = '$hrem' WHERE `leave_history`.`staff_id` = '$hodid';");
    }
    else if($tol=="Commuted"){
        $nod=2*$nod;
        $htak=$htak-$nod;
        $hcrem=$hcrem+$nod;
        mysqli_query($con,"UPDATE `leave_history` SET `half_taken` = '$htak', `half_remaining` = '$hcrem' WHERE `leave_history`.`staff_id` = '$hodid';");
        
    }
    else if($tol=="mat/pat"){
        $mtak=$mtak-$nod;
        $mrem=$mrem+$nod;
        mysqli_query($con,"UPDATE `leave_history` SET `mat/pat_taken` = '$mtak', `mat/pat_remaining` = '$mrem' WHERE `leave_history`.`staff_id` = '$hodid';");
    }
...
//}
   ...
   mysqli_query($con," UPDATE leave_requests SET status = 'hodcancelled' WHERE app_no = '$appno';");
}
}      
}
else{
